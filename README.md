...
- 空闲且无目标的 Basic、Evolved、Hyper 感染体每 4 Tick执行一次非运行 Goal 的启动/清理检查；已运行 Goal、战斗、受击和目标失效仍走原版逐 Tick路径。
- `[refactor.navigation]` 默认启用路径请求合并和短期原生路径共享。灾厄追击在真正进入 `createPath` 前复用活动路径，同 Tick 重复请求直接合并，并按实体稳定错峰为 10–20 Tick刷新；目标移动超过 4 格、新受击或卡住恢复会立即解除等待。复用路径会从实体当前位置附近的前向节点继续，不会每 Tick 回到缓存路径的旧起点；无法抵达目标的部分路径不进入缓存。普通地面单位的长路径可使用不可变方块快照在后台生成粗走廊，最终局部路径仍由实体导航器在服务端线程验证。
- Hybrid、Calamity、飞行、水下、攀爬和地下导航均享受请求合并；不适合二维快照的特殊导航自动保留原NodeEvaluator。粗走廊默认每维度每 Tick最多建立一个主线程方块快照，并在下一个快照边界受 750 微秒预算约束。
...
- 真菌后台时间预算：菌丝侵蚀和卷须搜索各有独立上限，并额外共享 `900` 微秒/Tick 的硬上限；两个队列每 Tick轮换优先级，每 16 个位置检查一次时间，达到上限后保留游标。
- 群体感知：相同目标的邻居查询共享 5 Tick 空间 UUID 快照；Follow 伙伴使用 20 Tick 单元快照，并把后续原生 20 Tick 搜索按 UUID 错峰。首次搜索仍即时。
- 伙伴跟随路径：`followPathReuse` 复用仍有效且目标移动不足 2 格的路径，默认周期刷新为 40 Tick并按 UUID 错峰；共享走廊和“路径已结束”不再绕过该租约。`followPathFailureBackoff` 可独立启用 20/40/80 Tick失败退避，伙伴变更和明显移动会立即解除等待。
...
- Howitzer 后台扫描：每 200 Tick触发的约 3.8 万格矿物搜索改为只读已加载区块的增量任务，默认单体 `1024`、全局 `4096` 方块/Tick并受 `500` 微秒硬预算限制；开火前可燃方块统计取消临时列表分配。
